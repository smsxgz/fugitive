...
from fugitive.rules import PILE_CARDS
...
def test_boolean_consistency_query_rejects_malformed_external_worlds() -> None:
    _world, observation = _world_and_observation()

    assert not is_complete_world_consistent(object(), observation)  # type: ignore[arg-type]
...
@pytest.mark.parametrize("history_name", ("draw_history", "guess_history"))
def test_public_history_rounds_must_be_valid(history_name: str) -> None:
    world, observation = _world_after_normal_guess()
    history = getattr(observation, history_name)
    malformed = (replace(history[0], round_number=-1), *history[1:])

    with pytest.raises(CompleteWorldConsistencyError, match="invalid round"):
        validate_complete_world(
            world,
            replace(observation, **{history_name: malformed}),
        )
