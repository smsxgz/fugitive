...
from fugitive.agents.policy_mixture import WeightedPolicySpec, default_policy_spec
from fugitive.agents.rollout_bir2u_marshal import RolloutBIR2UMarshalAgent
from fugitive.driver import GuessDecision
...
def test_same_seed_produces_the_same_rollout_distribution():
    observation = _marshal_guess_observation()

    first = _tiny_agent(2026).guess_distribution(observation)
    second = _tiny_agent(2026).guess_distribution(observation)
...
def test_marshal_rollout_exposes_its_budgeted_identity() -> None:
    shared = {
        "particle_count": 1,
        "opponent_policies": (_hierarchical_fugitive_spec(),),
    }
    agent = RolloutBIR2UMarshalAgent(1, **shared)
