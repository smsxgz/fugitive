...
from fugitive.agents.marshal_candidates import build_marshal_guess_catalogue
from fugitive.agents.route_count_catalogue_random import (
    RouteCountCatalogueRandomMarshalAgent,
)
from fugitive.agents.support_catalogue_random import (
...
@pytest.mark.parametrize(
    "kwargs",
    (
        {"multi_guess_continuation": 1.1},
        {"max_guess_size": 0},
        {"max_joint_candidates": 0},
    ),
)
def test_invalid_controlled_parameters_are_rejected(kwargs) -> None:
    with pytest.raises(ValueError):
        SupportCatalogueRandomMarshalAgent(**kwargs)


def test_only_route_count_catalogue_exposes_epsilon() -> None:
    with pytest.raises(TypeError, match="epsilon"):
        SupportCatalogueRandomMarshalAgent(epsilon=0.5)
    with pytest.raises(ValueError, match="epsilon"):
        RouteCountCatalogueRandomMarshalAgent(epsilon=-0.1)
